...
async function buscarMensajePorSufijo(sufijo: string): Promise<{ id: string; conversacion_id: string } | null> {
  try {
    // 1. Primero buscar directamente por el ID (Evolution API)
    ...
    // 2. Si no encuentra, buscar por sufijo (Cloud API con wamid)
...
app.post('/webhook/evolution/administracion', async (req: Request, res: Response) => {
  try {
    ...
    // 2. NORMALIZAR PAYLOAD (desde n8n o directo de Evolution)
    let payload;
    if (body.telefono) {
      payload = {
    ...
    // 3. VALIDAR - permitir mensaje vacío si hay multimedia
    if (!payload || !payload.telefono) {
      res.json({ success: true, ignored: true, reason: 'Sin teléfono' });
      return;
    }
    
    // Si no hay mensaje pero hay media, poner placeholder
