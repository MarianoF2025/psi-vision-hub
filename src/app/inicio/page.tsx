...
import { Eye, MessageSquare, ChevronRight, Lightbulb, Home, BarChart3, LogOut, User, Menu, X, Loader2 } from 'lucide-react';
...
const SIDEBAR_ITEMS = [
  { id: 'inicio', nombre: 'Inicio', icono: Home, href: '/inicio', activo: true },
...
export default function InicioPage() {
  const { user, signOut, loading, tieneAccesoVisionHub } = useAuth();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const userName = user?.user_metadata?.nombre || user?.user_metadata?.full_name || user?.email?.split('@')[0] || null;

  // Proteger la página - solo usuarios con acceso a Vision Hub
  useEffect(() => {
    if (!loading && !user) {
      window.location.href = '/';
    } else if (!loading && user && !tieneAccesoVisionHub) {
      window.location.href = '/crm';
    }
  }, [user, loading, tieneAccesoVisionHub]);

  // Loading mientras verifica
  if (loading || !user || !tieneAccesoVisionHub) {
    return (
