...
class PaymentController {
    ...
    async getProviders(req, res) {
        try {
            const providers = PaymentService_1.paymentService.getProviderInfo();
    ...
    async listarPagos(req, res) {
        try {
            ...
            const pagos = await PaymentService_1.paymentService.obtenerTodosLosPagos({
                estado: estado,
                provider: provider,
                desde: desde,
                hasta: hasta,
    ...
    async crearPago(req, res) {
        try {
            ...
            const pago = await PaymentService_1.paymentService.crearPago(request);
            res.json({
                success: true,
                pago,
    ...
    async obtenerPago(req, res) {
        try {
            const { id } = req.params;
            const pago = await PaymentService_1.paymentService.obtenerPago(id);
            if (!pago) {
     ...
     */
    async obtenerPagosPorConversacion(req, res) {
        try {
            const { id } = req.params;
            const pagos = await PaymentService_1.paymentService.obtenerPagosPorConversacion(id);
            res.json({ success: true, pagos });
        }
        catch (error) {
    ...
    async cancelarPago(req, res) {
        try {
            const { id } = req.params;
            const pago = await PaymentService_1.paymentService.cancelarPago(id);
            res.json({ success: true, pago });
        }
        catch (error) {
    ...
    async webhookMercadoPago(req, res) {
        try {
            ...
            const result = await PaymentService_1.paymentService.procesarWebhook('mercadopago', req.body);
    ...
    async webhookStripe(req, res) {
        try {
            ...
            const result = await PaymentService_1.paymentService.procesarWebhook('stripe', req.body, req.headers);
            res.json({ success: result.success, pago: result.pago });
        }
        catch (error) {
     ...
     * POST /webhook/siro
     */
    async webhookSiro(req, res) {
        try {
            console.log('[Webhook SIRO] Recibido:', JSON.stringify(req.body));
            const result = await PaymentService_1.paymentService.procesarWebhook('siro', req.body);
            res.json({ success: result.success, pago: result.pago });
        }
        catch (error) {
     ...
     * POST /webhook/dlocal
     */
    async webhookDLocal(req, res) {
        try {
            console.log('[Webhook DLocal] Recibido:', JSON.stringify(req.body));
            const result = await PaymentService_1.paymentService.procesarWebhook('dlocal', req.body, req.headers);
            res.json({ success: result.success, pago: result.pago });
        }
        catch (error) {
