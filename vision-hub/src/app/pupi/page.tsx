...
export default function PupyPage() {
  ...
  const enviarMensaje = async () => {
    ...
    try {
      // Construir mensajes para el API
      const mensajesParaApi = [...mensajes, mensajeUsuario]
        .filter(m => m.id !== mensajes[0]?.id || mensajes[0]?.rol !== 'assistant') // Excluir briefing del historial si es el primero
        .map(m => {
          const msg: any = { rol: m.rol, contenido: m.contenido };

          // Si tiene archivos, construir contenido multimodal
  ...
  const fileToBase64 = (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        resolve(result.split(',')[1]); // Quitar "data:xxx;base64,"
      };
  ...
  const nuevaConversacion = async () => {
    // Guardar conversación actual si tiene >2 mensajes
  ...
  const renderMarkdown = (text: string): string => {
    return text
      ...
      .join('')
      // Bold e italic dentro de líneas
