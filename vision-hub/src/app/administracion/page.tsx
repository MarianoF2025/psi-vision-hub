...
export default function AdministracionPage() {
  ...
  const previewMetrics = [
    { icon: DollarSign, label: 'Facturación', value: '—', color: '#10b981' },
    { icon: CreditCard, label: 'Cobros Pendientes', value: '—', color: '#f59e0b' },
    { icon: TrendingUp, label: 'LTV Promedio', value: '—', color: '#3b82f6' },
    { icon: PiggyBank, label: 'Margen Neto', value: '—', color: '#8b5cf6' },
  ];

  const upcomingFeatures = [
    { icon: DollarSign, text: 'Facturación ARS y USD' },
    { icon: CreditCard, text: 'Estado de cobros (SIRO, MP, Dlocal)' },
    { icon: TrendingUp, text: 'LTV por segmento' },
    { icon: Receipt, text: 'Morosidad y aging' },
    { icon: Calculator, text: 'Proyecciones de ingresos' },
    { icon: PiggyBank, text: 'Márgenes por curso' },
  ];
      ...
      <div className="p-3 sm:p-4 lg:p-6 space-y-3 sm:space-y-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8 lg:p-10">
          <div className="text-center max-w-lg mx-auto">
            <div className="inline-flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-2xl mb-4">
              <Construction className="w-7 h-7 sm:w-8 sm:h-8 text-blue-600" />
            </div>
            <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-2">En Construcción</h2>
            <p className="text-xs sm:text-sm text-gray-500 mb-6">Este dashboard conectará con las pasarelas de pago para métricas financieras.</p>
            ...
            <div className="mt-6 flex items-center justify-center gap-2 text-gray-400">
              ...
              <span className="text-[10px] sm:text-xs">Datos desde SIRO, MercadoPago, Dlocal</span>
            </div>
          </div>
        </div>
      </div>
