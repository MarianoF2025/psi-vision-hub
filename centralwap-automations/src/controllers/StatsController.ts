...
function getDateRange(desde?: string, hasta?: string, periodo?: string): { inicio: Date; fin: Date } {
  const ahora = new Date();
  let inicio: Date;
  let fin: Date = hasta ? new Date(hasta + 'T23:59:59') : new Date(ahora.getFullYear(), ahora.getMonth(), ahora.getDate(), 23, 59, 59);
...
export class StatsController {
  ...
  async dashboard(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { inicio, fin } = getDateRange(desde as string, hasta as string, periodo as string);

      // Total leads en período
      const { count: leadsPeriodo } = await supabase.from('menu_sesiones')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', inicio.toISOString())
        .lte('created_at', fin.toISOString());

      // Leads por origen
      const { count: leadsCTWA } = await supabase.from('menu_sesiones')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', inicio.toISOString())
        .lte('created_at', fin.toISOString())
        .eq('origen', 'ctwa');
      
      const { count: leadsDirectos } = await supabase.from('menu_sesiones')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', inicio.toISOString())
        .lte('created_at', fin.toISOString())
        .eq('origen', 'directo');
      ...
      // Interacciones
      const { count: interaccionesPeriodo } = await supabase.from('menu_interacciones')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', inicio.toISOString())
        .lte('created_at', fin.toISOString());

      // Leads con interacción
      const { data: sesionesEngaged } = await supabase.from('menu_sesiones')
        .select('id')
        .gte('created_at', inicio.toISOString())
        .lte('created_at', fin.toISOString())
        .gt('interacciones', 0);
      ...
      const { count: cursosActivos } = await supabase.from('cursos')
        .select('*', { count: 'exact', head: true })
        .eq('activo', true);
      
      const { count: anunciosActivos } = await supabase.from('config_cursos_ctwa')
        .select('*', { count: 'exact', head: true })
        .eq('activo', true);
  ...
  async ctrPorOpcion(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { data: opciones } = await supabase.from('menu_opciones')
        .select('*')
        .eq('curso_id', cursoId)
        .order('orden', { ascending: true });

      const { count: totalSesiones } = await supabase.from('menu_sesiones')
        .select('*', { count: 'exact', head: true })
        .eq('curso_id', cursoId)
        .gte('created_at', inicio.toISOString())
        .lte('created_at', fin.toISOString());
      ...
      const stats = await Promise.all((opciones || []).map(async (opcion) => {
        const { count: veces } = await supabase.from('menu_interacciones')
          .select('*', { count: 'exact', head: true })
  ...
  async automatizaciones(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { data: configs } = await supabase.from('config_cursos_ctwa')
      ...
      const stats = await Promise.all((configs || []).map(async (config) => {
        const { count: leads } = await supabase.from('menu_sesiones')
          .select('*', { count: 'exact', head: true })
        ...
        const { count: opcionesActivas } = await supabase.from('menu_opciones')
          .select('*', { count: 'exact', head: true })
  ...
  async interaccionesRecientes(req: Request, res: Response): Promise<void> {
    try {
      ...
      let query = supabase.from('menu_interacciones')
        .select('*')
  ...
  async detalleCurso(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { count: leadsPeriodoActual } = await supabase.from('menu_sesiones')
        .select('*', { count: 'exact', head: true })
        .eq('curso_id', cursoId)
        .gte('created_at', inicio.toISOString())
        .lte('created_at', fin.toISOString());
      ...
      const { count: leadsPeriodoAnterior } = await supabase.from('menu_sesiones')
        .select('*', { count: 'exact', head: true })
        .eq('curso_id', cursoId)
      ...
      const { count: leadsTotal } = await supabase.from('menu_sesiones')
        .select('*', { count: 'exact', head: true })
        .eq('curso_id', cursoId);
      ...
      const { data: sesionesConInteraccion } = await supabase.from('menu_sesiones')
        .select('id')
        .eq('curso_id', cursoId)
        .gt('interacciones', 0)
        .gte('created_at', inicio.toISOString())
      ...
      const opcionesConVeces = await Promise.all((opciones || []).map(async (opcion) => {
        const { count: vecesElegida } = await supabase.from('menu_interacciones')
          .select('*', { count: 'exact', head: true })
          .eq('opcion_id', opcion.id)
          .gte('created_at', inicio.toISOString())
          .lte('created_at', fin.toISOString());
      ...
      const { data: anuncios } = await supabase.from('config_cursos_ctwa')
        .select('id, ad_id, nombre, activo, ejecuciones')
        .eq('curso_id', cursoId);

      const anunciosConStats = await Promise.all((anuncios || []).map(async (anuncio) => {
        const { count: leadsAnuncio } = await supabase.from('menu_sesiones')
          .select('*', { count: 'exact', head: true })
          .eq('config_ctwa_id', anuncio.id)
          .gte('created_at', inicio.toISOString())
          .lte('created_at', fin.toISOString());

        const { data: sesionesAnuncio } = await supabase.from('menu_sesiones')
          .select('id')
          .eq('config_ctwa_id', anuncio.id)
          .gt('interacciones', 0)
          .gte('created_at', inicio.toISOString())
        ...
        const { data: interaccionesAnuncio } = await supabase.from('menu_interacciones')
          .select('opcion_id')
          .eq('config_ctwa_id', anuncio.id)
          .gte('created_at', inicio.toISOString())
          .lte('created_at', fin.toISOString());
