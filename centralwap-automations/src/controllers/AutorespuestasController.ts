...
class AutorespuestasController {
  ...
  async verificar(req: Request, res: Response) {
    try {
      ...
      const { data: respuestasAgente } = await supabase
        .from('mensajes')
      ...
      const mensajeKey = `mensaje_franja_${franja}` as keyof typeof config;
      let mensaje = config[mensajeKey] as string;
      ...
      await supabase.from('autorespuestas_enviadas').insert({
        telefono,
        linea,
        franja,
        mensaje
   ...
   */
  private determinarFranja(corteActivo: boolean): number {
    if (corteActivo) {
   ...
   */
  private procesarVariables(mensaje: string): string {
    const ahora = new Date();
    const horaArgentina = new Date(ahora.toLocaleString('en-US', { timeZone: 'America/Argentina/Buenos_Aires' }));
    const hora = horaArgentina.getHours();
...
export const autorespuestasController = new AutorespuestasController();
