...
export class CursosController {
  ...
  async crear(req: Request, res: Response): Promise<void> {
    try {
      ...
      await auditService.logCursoCreado(data.id, data, userName);
  ...
  async actualizar(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { data: anterior } = await supabase.from('cursos').select('*').eq('id', id).single();
      if (!anterior) { res.status(404).json({ success: false, error: 'Curso no encontrado' }); return; }
      ...
      await auditService.logCursoActualizado(id, anterior, body, userName);
  ...
  async eliminar(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { data: existing } = await supabase.from('cursos').select('id, codigo, nombre').eq('id', id).single();
      if (!existing) { res.status(404).json({ success: false, error: 'Curso no encontrado' }); return; }
      
      const { error } = await supabase.from('cursos').delete().eq('id', id);
      if (error) throw error;
      
      await auditService.logCursoEliminado(id, existing, userName);
  ...
  async toggle(req: Request, res: Response): Promise<void> {
    try {
      ...
      await auditService.logCursoToggle(id, existing.codigo, data.activo, userName);
