...
class MenuController {
    ...
    extraerNombreCursoDeWeb(mensaje) {
    ...
    async matchearCursoPorNombre(nombreWeb) {
        ...
        const { data: cohortes } = await supabase_1.supabase
        ...
        if (cohortes) {
            ...
            if (mejorMatch) {
                ...
                const { data: curso } = await supabase_1.supabase
                    .from('cursos')
                    .select('*')
        ...
        const { data: todosLosCursos } = await supabase_1.supabase
            .from('cursos')
            .select('*')
            .order('codigo');
        if (todosLosCursos) {
            ...
            if (nombreCursoWeb) {
                ...
                if (curso && curso.activo) {
                    ...
                    if (opciones?.length) {
                        ...
                        await WhatsAppService_1.whatsAppService.enviarTexto(telefono, `¡Hola! 👋 Gracias por tu interés en *${curso.nombre}*.\n\nEn breve te contacta un asesor con toda la info.`);
                    ...
                    await WhatsAppService_1.whatsAppService.enviarTexto(telefono, `¡Hola! 👋 Gracias por contactarnos.\n\nEn breve te atiende un asesor.`);
        ...
        try {
            ...
            if (cursos.length <= MAX_ROWS_WA) {
                ...
                const sections = Object.entries(porCat).map(([cat, cs]) => ({
                    ...
                    rows: cs.map((c) => ({ id: `curso_${c.id}`, title: c.nombre.substring(0, 24), description: c.descripcion?.substring(0, 72) || '' }))
                }));
